...
const BookAppointment = ({date}) => {
    ...
    return (
        <section className="container">
            <h2 className="text-primary text-center">Available Appointments on {date.toDateString()}</h2>
            <div className="row mt-5">
                {
                    bookingData.map(bookings => <BookingCard bookings={bookings} date={date}></BookingCard>)
                }
            </div>
        </section>
    );
